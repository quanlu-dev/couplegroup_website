...
(function () {
  ...
  function renderHeader() {
    ...
    var html =
      ...
      '<div class="flex justify-between items-center w-full px-margin-mobile lg:px-margin-desktop py-4 max-w-container-max mx-auto">' +
      ...
      "</a>" +
      '<nav class="hidden lg:flex space-x-8">' + buildDesktopNav(current) + "</nav>" +
      ...
      '<div id="mobile-drawer" class="fixed top-0 right-0 h-screen w-[85%] max-w-sm bg-white shadow-2xl z-[101] translate-x-full transition-transform duration-300 flex flex-col">' +
      '<div class="flex items-center justify-between p-6 border-b">' +
      '<div class="flex items-center"><img src="assets/logo-removebg-preview.png" class="h-8 w-auto"><span class="ml-3 font-bold text-ink-black">Couple Group</span></div>' +
      '<button id="menu-close" class="p-2 rounded-full hover:bg-gray-100"><span class="material-symbols-outlined text-3xl">close</span></button>' +
      "</div>" +
      '<nav class="flex-1 overflow-y-auto">' + buildMobileNav(current) + "</nav>" +
      "</div>" +
      "</header>";
